...
describe(evaluateScript, () => {
  it("returns undefined if there was no event", async () => {
    ...
    expect(await runScript({ type: "Query.unknown", accessToken: '', args: {}, parents: null})).toBeUndefined()
  })
  ...
  it("returns the value if there is a resolver registered", async () => {
    ...
    expect(await runScript({ type: "Query.fortyTwo", accessToken: '', args: {}, parents: null })).toEqual(42)
  })
  ...
  it("passes the args and parents over", async () => {
    ...
    expect(await runScript({ type: "User.fortyTwo", accessToken: '', args: {foo: 1}, parents: [{n: 41}] })).toEqual([42])
  })
  ...
  it("returns undefined if the number of parents doesn't match the number of return types", async () => {
    ...
    expect(await runScript({ type: "Query.fortyTwo", accessToken: '', args: {}, parents: null })).toBeUndefined()
  })
  ...
  it("returns undefined somehow the script doesn't return an array", async () => {
    ...
    expect(await runScript({ type: "User.fortyTwo", accessToken: '', args: {}, parents: [{n: 42}] })).toBeUndefined()
  })
  ...
  integrationTest("dgraph integration", () => {
    ...
    it("works with dgraph graphql", async () => {
      ...
      const results = await runScript({ type: "Query.todoTitles", accessToken: '', args: {}, parents: null });
